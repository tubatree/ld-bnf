...
<topic>
<title>Carbamazepine</title>
<body><p><ph>Interaction one</ph>.<ph class="moderate" outputclass="int-severity">Moderate</ph></p></body>
</topic>"""
    let interaction = parseInteractionFrom xml
...
let ``When there is no severity in the feed, the build should fail`` () =
    ...
    let interaction = parseInteractionFrom xml
    Assert.AreEqual("Unknown", interaction.importance.ToString())


[<Test>]
let ``Should build interaction message from topic with more than one paragraph`` () =
    let xml = """
	<topic>
    ...
    let interaction = parseInteractionFrom xml
    Assert.AreEqual("This is a test. This is a second test.", interaction.message.XElement.Value)

[<Test>]
let ``Should build interaction message from topic with more than one paragraph and exclude severity`` () =
    let xml = """
	<topic>
			<title>Asenapine</title>
			<body><p>This <ph>is</ph> a <ph>test</ph>. </p><p>This <ph>is</ph> a second <ph>test</ph>.<ph class="%s" outputclass="int-severity">Not used</ph></p></body>
		</topic>"""
    let interaction = parseInteractionFrom xml
    Assert.AreEqual("This is a test. This is a second test.", interaction.message.XElement.Value)
...
let ``Should build interaction importance from topic with multiple paragraphs`` () =
	...
	<topic>
			<title>Asenapine</title>
			<body><p></p><p><ph class="severe" outputclass="int-severity">Not used</ph></p></body>
	</topic>"""
    let interaction = parseInteractionFrom xml
    Assert.AreEqual("Severe", interaction.importance.ToString())
